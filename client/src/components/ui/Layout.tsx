...
function ChangeMyPasswordModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (newPw.length < 6) { toast.error("Kam se kam 6 characters chahiye"); return; }
    setLoading(true);
    try {
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-orange-100">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-bold text-gray-900 text-sm leading-tight">{t("appName")}</p>
            ...
            <div className="absolute bottom-full left-0 right-0 mb-1 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden z-10">
              {(["hinglish", "hindi", "english"] as Language[]).map(l => (
                <button key={l} onClick={() => handleLangChange(l)}
                  className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 transition-colors ${lang === l ? "text-brand-600 font-medium bg-brand-50" : "text-gray-700"}`}>
                  {langLabels[l]}
                </button>
              ))}
            </div>
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="md:hidden bg-white border-b border-orange-100 px-4 py-3 flex items-center gap-3">
          ...
          <span className="font-bold text-gray-900">🐄 Smart Dairy</span>
        </header>
